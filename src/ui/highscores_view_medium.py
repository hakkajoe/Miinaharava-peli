...
class HighscoresViewMedium:
    def __init__(self, root, show_mainmenu_view, show_easyscores_view, show_hardscores_view):
        ...
        self._scores = score_service.get_scores_medium()
    ...
    def _start(self):
        ...
        self.button2 = ttk.Button(master=self._frame, text="Scores (hard)", command=self._show_hardscores_view)

        if len(self._scores) == 0:
            self.intro_label = ttk.Label(master=self._frame, text="No scores available")
        else:
            self.intro_label = ttk.Label(master=self._frame, text="Top 10 (medium):")
